...
public class BookFragment extends BaseRVFragment<RecommendPresenter, Recommend.RecommendBooks> implements RecommendContract.View, RecyclerArrayAdapter.OnItemLongClickListener {
...
//                        ((MainActivity) activity).setCurrentItem(2);
                        ((ZhuishuActivity) activity).gotoBookFindFragment();
...
//                                    mAdapter.getItem(position)._id);
    ...
    public void onRefresh() {
        ...
        if (!hasRefBookShelfInCallStack && isMRefresh) {
            ((ZhuishuActivity) activity).pullSyncBookShelf();
            return;
        }
